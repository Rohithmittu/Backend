...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordValid) {
    throw new ApiError(401, "password incorrect"); // chnage at the end of project "Invaalid credentials"
  }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const { accessToken, newRefreshToken } =
      await generate_Access_And_RefreshToken(user._id);
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Password changeed succesfully"));
});
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId(req.user._id), // we used like this because aggregation pipline code is taken directly,
        // so _id will no work so we shoul ent how the data is present in mongoDB  as it is
      },
    },
    {
      $lookup: {
